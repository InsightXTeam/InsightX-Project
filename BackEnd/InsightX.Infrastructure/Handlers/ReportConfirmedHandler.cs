...
namespace InsightX.Infrastructure.Handlers
{
    public class ReportConfirmedHandler : IReportConfirmedHandler
    {
        ...
        public async Task HandleAsync(int companyId, int? departmentId, string kpiName, decimal value, int? reportId = null)
        {
            await _generateAlert.ExecuteAsync(companyId, departmentId, kpiName, value, reportId);
        }
    }
}
